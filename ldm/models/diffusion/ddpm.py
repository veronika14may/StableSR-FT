...
def fft_magnitude_loss(pred, target):
    """FFT-loss из PiSA-SR: L1 между log-амплитудными спектрами.

    Возвращает вектор [B] (поэлементно по батчу), чтобы можно было
    маскировать отдельные сэмплы по шагу диффузии t.
    """
    p = pred.float()
    t = target.float()
    p_mag = torch.log1p(torch.abs(torch.fft.fft2(p, norm='ortho')))
    t_mag = torch.log1p(torch.abs(torch.fft.fft2(t, norm='ortho')))
    return (p_mag - t_mag).abs().mean(dim=[1, 2, 3])
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        # --- FFT-loss (по аналогии с PiSA-SR) ---
        self.lambda_fft = lambda_fft
        assert fft_space in ('latent', 'pixel'), fft_space
        self.fft_space = fft_space          # 'latent' — спектр латента, 'pixel' — спектр RGB после VAE-декодера
        self.fft_t_max_ratio = fft_t_max_ratio  # применять только при t < ratio * T (при больших t x0-предсказание бессмысленно)
        self.fft_warmup = fft_warmup        # линейный разогрев веса, в шагах оптимизатора
        self.fft_patch = fft_patch          # размер кропа латента для pixel-режима (0 = без кропа)
        assert self.num_timesteps_cond <= kwargs['timesteps']
        # for backwards compatibility after implementation of DiffusionWrapper
        if conditioning_key is None:
            conditioning_key = 'concat' if concat_mode else 'crossattn'
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):
        ...
        # ----- FFT-loss (перенос из PiSA-SR) -----
        if self.lambda_fft > 0:
            loss_fft_raw, lam_fft_now = self.get_fft_loss(model_output_, x_noisy, x_start, t)
            loss_dict.update({f'{prefix}/loss_fft': loss_fft_raw})
            loss = loss + lam_fft_now * loss_fft_raw

        loss_dict.update({f'{prefix}/loss': loss})

        return loss, loss_dict

    def predict_xstart_from_output(self, model_output, x_noisy, t):
        """Предсказание x0 из выхода UNet с учётом параметризации модели."""
        if self.parameterization == "eps":
            return self.predict_start_from_noise(x_noisy, t=t, noise=model_output)
        elif self.parameterization == "x0":
            return model_output
        elif self.parameterization == "v":
            return self.predict_start_from_z_and_v(x_noisy, model_output, t)
        raise NotImplementedError()

    def get_fft_loss(self, model_output, x_noisy, x_start, t):
        """FFT-loss на предсказании x0.

        Возвращает (сырое значение лосса, текущий вес с учётом warmup).
        В отличие от PiSA-SR (одношаговая модель, pred сразу в пикселях),
        здесь x0 восстанавливается из выхода UNet, а сэмплы с большим t
        отбрасываются: при t -> T предсказание x0 -- почти шум и спектр
        по нему неинформативен.
        """
        zero = torch.zeros((), device=x_start.device)

        if self.fft_warmup > 0:
            lam = self.lambda_fft * min(1.0, self.global_step / float(self.fft_warmup))
        else:
            lam = self.lambda_fft
        if lam <= 0:
            return zero, 0.0

        t_max = int(self.fft_t_max_ratio * self.num_timesteps)
        keep = t < t_max
        if not bool(keep.any()):
            return zero, lam
        idx = torch.nonzero(keep, as_tuple=False).squeeze(1)

        x0_pred = self.predict_xstart_from_output(model_output, x_noisy, t)[idx]

        if self.fft_space == 'latent' or not hasattr(self, 'gt'):
            loss_vec = fft_magnitude_loss(x0_pred, x_start[idx])
        else:
            gt_pix = self.gt[:x_start.size(0)][idx]
            sf = gt_pix.size(-1) // x0_pred.size(-1)  # обычно 8 (VAE)
            if 0 < self.fft_patch < x0_pred.size(-1):
                ph = self.fft_patch
                top = random.randint(0, x0_pred.size(-2) - ph)
                left = random.randint(0, x0_pred.size(-1) - ph)
                x0_pred = x0_pred[:, :, top:top + ph, left:left + ph]
                gt_pix = gt_pix[:, :, top * sf:(top + ph) * sf, left * sf:(left + ph) * sf]
            pred_pix = self.differentiable_decode_first_stage(x0_pred)
            loss_vec = fft_magnitude_loss(pred_pix, gt_pix)

        return loss_vec.mean(), lam
...
class LatentDiffusionSRTextWTPaired(LatentDiffusionSRTextWT):
    """Вариант для готовых LQ/HQ пар (свои данные, а не синтетика Real-ESRGAN).

    Отличия от родителя:
      * пайплайн деградаций (blur/noise/JPEG/resize) не запускается -- LQ берётся из батча;
      * не используется training pair pool (_dequeue_and_enqueue): он нужен, чтобы
        размешать синтетические деградации внутри батча, для реальных пар смысла нет;
      * USM-шарпенинг GT по умолчанию стоит выключать (use_usm: False) -- на реальных
        парах он ломает соответствие LQ-HQ.

    Ожидает батч с ключами 'lq' и 'gt': RGB, CHW, диапазон [0, 1]
    (ровно то, что отдаёт basicsr.data.realesrgan_paired_dataset.RealESRGANPairedDataset).
    """
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        self.gt = torch.clamp(im_gt, 0, 1.0)
        self.lq = torch.clamp(im_lq, 0, 1.0)

        # LQ приводится к разрешению GT: StableSR подаёт в UNet латент того же размера
        if resize_lq and self.lq.shape[-2:] != self.gt.shape[-2:]:
            self.lq = F.interpolate(
                    self.lq,
